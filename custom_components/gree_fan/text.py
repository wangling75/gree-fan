...
def _format_minutes(total_minutes: float | None) -> str:
    """分钟数格式化为 HH:MM。"""
...
async def async_setup_entry(
    ...
    def init_device(coordinator):
        ...
        if all(coordinator.supports(prop) for prop in timer_props):
...
class GreeTimerText(GreeEntity, TextEntity):
    ...
    def native_value(self) -> str:
        ...
        if fan is None:
            _LOGGER.warning("[timer] native_value: 找不到风扇实体，返回 00:00")
            return "00:00"
        
        minutes = fan.timer_minutes
        result = _format_minutes(minutes)
        _LOGGER.debug("[timer] native_value: timer_minutes=%.2f, 返回 %s", minutes, result)
        return result

    async def async_set_value(self, value: str) -> None:
        """解析用户输入为分钟数并下发到风扇实体。"""
        _LOGGER.info("[timer] async_set_value: 收到输入 %r", value)
        fan = self._get_fan_entity()
        if fan is None:
            _LOGGER.warning("[timer] async_set_value: 找不到风扇实体")
            return
        minutes = _parse_timer_input(value)
        if minutes == -1:
            _LOGGER.warning("[timer] 输入格式无效: %r", value)
            return
        _LOGGER.info("[timer] 调用 fan.async_set_timer_minutes(%s)", minutes)
        await fan.async_set_timer_minutes(minutes)
        self.async_write_ha_state()
        _LOGGER.info("[timer] 设置完成，当前 timer_minutes=%.2f", fan.timer_minutes)

    def _get_fan_entity(self):
        """通过 MAC 地址查找对应的风扇实体。"""
        mac = self.coordinator.device.device_info.mac
        fan = self.hass.data.get(DOMAIN, {}).get("fans", {}).get(mac)
        
        if fan is None:
            _LOGGER.debug(
                "[timer] _get_fan_entity: MAC=%s, fans dict=%s",
                mac,
                list(self.hass.data.get(DOMAIN, {}).get("fans", {}).keys()),
            )
