    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            if not self.last_update_success or self._error_count >= MAX_ERRORS:
                ...
                if self._coordinators is not None and self in self._coordinators:
                    self._coordinators.remove(self)
                    _LOGGER.info(
                        "Removed unavailable device from coordinators list: %s (MAC=%s)",
                        self.name,
                        self.device.device_info.mac,
                    )
                raise UpdateFailed(f"Device {self.name} is unavailable") from error
...
class DiscoveryService(Listener):
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self._coordinators = hass.data[DOMAIN].setdefault(COORDINATORS, [])
    ...
    async def device_found(self, device_info: DeviceInfo) -> None:
        ...
        # MAC 去重：已存在的设备尝试重新 bind（断电重启后 IP/key 可能变化）
        existing = None
        ...
        if existing is not None:
            _LOGGER.info(
                "Re-binding existing device: %s (MAC=%s) after power cycle",
                device_info.name, device_info.mac,
            )
            try:
                await existing.device.bind()
                await existing.async_refresh()
                async_dispatcher_send(self.hass, DISPATCH_DEVICE_DISCOVERED, existing)
                return
            except (DeviceNotBoundError, DeviceTimeoutError) as e:
                _LOGGER.warning(
                    "Re-bind failed for %s (MAC=%s), removing from list: %s",
                    device_info.name, device_info.mac, e,
                )
                self._coordinators.remove(existing)
                return
        ...
        coordinator = DeviceDataUpdateCoordinator(self.hass, device, profile, self._coordinators)
