...
class DeviceDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            # greeclimate 1.4.1 只轮询空调的标准字段；风扇的 Rotate 与
            # LRAngle 不在其 Props 枚举内，需要单独读取并合并状态。
            fan_swing_state = await network.request_state(
                [PROP_ROTATE, PROP_LR_ANGLE],
                self.device.device_info,
                self.device.device_key,
            )
            self.device._properties.update(fan_swing_state)
